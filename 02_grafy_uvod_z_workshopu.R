...
# Úkol
# Udělejte histogram pomocí ggplotu k ditribuci věku 
d %>% 
  # drop_na(vek) %>% 
  ggplot(aes(x = vek)) + 
...
ggplot(data = d, aes(x=vek, y = obvykla_delka_spanku, color = pohlavi)) +
  geom_point(shape = 5, size = 2, alpha =0.7) +
  theme_bw()
...
d %>% 
  ...
  theme_classic() +  
  geom_smooth(method = "lm", color = "darkgreen",  linetype = "dotted", size = 0.5)  # Přidá regresní čáru (výchozí typ "solid")
...
ggplot(data = d, aes(x = pohlavi)) +
  geom_bar(fill = "blue", color = "orange", width = 0.8) +  # Určí šířku sloupců na 0.5
...
# nejdřív si musíme průměrný věk pro obě pohlaví spočítat 
vek_muzi_zeny <- d %>%
  group_by(pohlavi) %>%
