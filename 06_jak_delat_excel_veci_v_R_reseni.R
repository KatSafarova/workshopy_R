...
t <- t %>% 
  mutate(n_znaku_tel= nchar(telefon_upraveny))  

t <- t %>%
  mutate(n_znaku_tel_bm1 = nchar(gsub(" ", "", telefon_upraveny)))

# alternativně s balíčkem stringr 
t <- t %>%
  mutate(n_znaku_tel_bm2 = nchar(str_replace_all(telefon_upraveny, " ", "")))
