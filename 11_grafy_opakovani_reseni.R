...
# pocet je potřeba nejdříve spočítat viz níže
...
d_pocty_zanik <- d |> 
  drop_na(zpusob_zaniku)|> 
  group_by(zpusob_zaniku) |> 
  summarise(pocet = n())

# geom_col + geom_line
ggplot(data = d_pocty_zanik, aes(y = pocet, x = zpusob_zaniku)) + 
  geom_col(fill = "#34ebc3", color = "darkblue") +
  labs(title = "Ekonomické subjekty podle zániku",
       x = "Způsob zániku",
       y = "Počty",
       caption = "Data k 31. 1. 2025, pouze vzorek 10 000 ekonomických subjektů.") +
  theme_minimal() + 
...
ggplot(data, aes(x = question, y = percent, fill = fct_rev(answer))) +
  geom_col(position = "stack", color = "black", linewidth = 0.02) +
  geom_text(
    aes(label = ifelse(percent > 5, paste0(percent, " %"), ""),
        color = fct_rev(answer)), # Přidání estetiky color podle odpovědi)
...
# v našem případě potřebujeme otočit pořadí faktorů, stačí přidat v aes na začátku  mezi fill = answer), fct_rev: ggplot(data, aes(x = question, y = percent, fill = fct_rev(answer))), což otočí pořadí původních levels ve faktoru; fct_rev taky přidatt v color(answer) v geom_tex: color = fct_rev(answer))
...
# vektor okresů s počtem nad 100 
okresy100 <- unique(d_okresy$okres)

d_okresy_pr_forma <- d |> 
  select(okres, pr_forma_org_res) |> 
  drop_na(okres, pr_forma_org_res) |> 
  filter(pr_forma_org_res %in% c("Společnost s ručením omezeným", "Fyzická osoba podnikající dle živnostenského zákona")) |> 
  mutate(pr_forma_org_res = if_else(pr_forma_org_res == "Společnost s ručením omezeným", "s.r.o", "FO dle ŽZ")) |> 
  filter(okres %in% okresy100) |> 
  group_by(okres, pr_forma_org_res) |> 
  summarise(pocet = n()) 


d_okresy |> 
  mutate(okres = as.factor(okres)) |> 
  mutate(okres = fct_reorder(okres, -pocet)) |> # změna faktorů podle pocet sestupně
  ggplot(aes(x = okres, y = pocet)) +  
  geom_col(fill = modra) +  # Sloupce vedle sebe
  labs(title = "Top okresy dle počtu ekonomických subjektů",
       x = "Okres",
       y = "Počet subjektů",
       fill = NULL) +
  theme(
...
d_okresy_pr_forma <- d_okresy_pr_forma |> 
  group_by(okres) |> 
  mutate(total_pocet = sum(pocet)) |>  # Spočítá celkový počet subjektů v okrese
  ungroup() |> 
  mutate(okres = fct_reorder(okres, total_pocet, .desc = TRUE))  # Řazení podle celkového součtu

ggplot(d_okresy_pr_forma, aes(x = okres, y = pocet, fill = pr_forma_org_res)) +  
  geom_col(position = "dodge", alpha = 0.7) +  
  scale_fill_manual(values = c("s.r.o" = "red", "FO dle ŽZ" = "#00BFFF")) +  
  labs(title = "Top okresy dle počtu vybraných ekonomických subjektů",
       x = "Okres",
       y = "Počet subjektů",
       fill = "Právní forma") +
  theme(
    legend.position = "top",
    axis.text.x = element_text(angle = 45, hjust = 1))  # Rotace textu pro lepší čitelnost
