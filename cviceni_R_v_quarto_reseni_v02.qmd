...
data <- data %>%
  mutate(vek = str_sub(vek_kod, -2, -1)) |>  # vezme poslední dvě číslice, správně by se mělo rozlišit ještě 99 (končící 099) a 100 a více (končící 999) - to jsem pro zjednodušení neřešila
  filter(vek > 17) |> 
...
# získání věku pomocí gsub 
# data <- data %>%
#   mutate(vek = gsub("^.*(..)$", "\\1", vek_kod))
...
Respondenti uváděli následující stavy (řazeno dle četnosti):

```{r}
#| label: pocty
#| echo: false

# Seřazení podle sloupce `hodnota` sestupně
rstavy_serazene <- souhrn_cr %>%
  arrange(desc(hodnota))  # Řazení podle hodnot sestupně

# Generování seznamu s vlastním znakem
custom_bullet <- "→"  # Zde nastavte libovolný znak
cat(paste(custom_bullet, rstavy_serazene$stav_txt, sep = " ", collapse = "\n"))
